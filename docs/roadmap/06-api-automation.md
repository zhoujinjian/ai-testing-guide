---
title: 06 接口测试与接口自动化
description: 接口自动化是测开最应该优先拿下的一块：接口比 UI 更稳定、执行更快、更容易接入 CI。
---
...
| 代码层 | pytest-mock / Mockito（见[第 03 章](./03-programming.md)） | 单测里 mock 函数、SDK、未就绪模块 |
...
3. 支付回调这类外部依赖，什么时候 Mock、什么时候必须真调？Mock 响应怎么和接口文档保持同步？
4. 用例幂等怎么做？连续跑两遍结果一致吗？
5. 接口需要签名（参数排序 + MD5），你的封装怎么设计？
6. 水平越权的测试用例怎么写？自动化里怎么覆盖？
7. Allure 报告里，一个失败的用例能看到哪些信息才算合格？
8. CI 上接口测试失败的三大类原因（环境、数据、代码）你怎么快速区分？
9. 让 AI 根据 Swagger 生成用例初稿，你审查的清单有哪些？
...
- [AI 测试开发导航 · 精选课程](https://www.testfather.cn/courses)：接口自动化实战教程，与本章框架六大件配套。
