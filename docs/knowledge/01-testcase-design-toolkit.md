---
title: 01 用例设计四件套：从需求拆解到质量评审
description: AI 测试实战系列第 01 篇。四款 Agent Skill 搭建「需求拆解 → 用例生成 → 场景补全 → 质量评审」全流程闭环：XMind 测试点生成、Excel 标准用例生成、高频遗漏场景智能补全、用例质量量化评审。
---

# AI 测试实战系列 01｜用例设计四件套：从需求拆解到质量评审

> AI 测试实战系列（共 19 篇）｜ 01 用例设计（接口/UI 通用） · 接口自动化 02-10 · UI 自动化 11-19 ｜ 本文第 01 篇
...
但在实际项目中，**线上问题、回归 Bug、隐蔽缺陷** 往往并不出现在正向主流程里，而是集中在**非功能性场景、极端边界条件、历史高频线上 Bug、业务隐藏规则**等人工最容易疏忽、AI 默认不会主动生成的薄弱区域。
